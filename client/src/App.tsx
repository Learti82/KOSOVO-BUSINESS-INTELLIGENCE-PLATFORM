        ...
        <nav className="flex gap-6 items-center text-sm">
          ...
          {user ? (
            <>
              ...
              {(user.role === 'analyst' || user.role === 'admin') && <Link to="/admin/orders">Admin</Link>}
              <button onClick={logout} className="text-slate-300 hover:text-white">Logout</button>
            </>
          ) : (
            <Link to="/login" className="bg-amber-500 text-slate-900 px-4 py-1 rounded font-medium">Login</Link>
          )}
        </nav>
      </div>
    </header>
